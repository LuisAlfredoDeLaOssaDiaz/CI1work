...
public class task {
    public static void main(String args[]) { // Funcion main
        ...
        cantidadEstudiantes = pedirCantidadEstudiantes(cantidadEstudiantes, read); // Apunta a la funcion pedirCantidadEstudiantesy la almacena en cantidadEstudiantes
        pass = guardarNotas(cantidadEstudiantes, read, tipoNota); // Apunta a la funcion guardarNotas y la almacena en pass
    ...
    static int guardarNotas(int cantidadEstudiantes, Scanner read, String tipoNota[]) {
        ...
        for (int i = 0; i < estudiante.length; i++) {
            System.out.println("****************************************************************************");
            for (int j = 0; j < tipoNota.length; j++) { // for para recorrer tipoNota respecto a la cantidad de estudianes
                j = (j > 3) ? 0 : j; // valida y se asegura que j no sea mayor que la cantidad de notas
                do {
                    System.out.print("Ingrese la nota del " + tipoNota[j] + " para el estudiante " + estudiante[i] + ": " ); // Imprime y pide el tipo de nota que requiere y el estudiante respectivo
                ...
                }while( pass == false ); // permanece en el bucle siempre y cuando pass sea falso hasta que la nota del estudiante sea correcta

                definitivaEstudiante[i] += notaEstudiante[i][j]; // Almacena la definitiva de los estudiantes en definitivaEstudiante
        ...
        int success = mostrarResultados(definitivaEstudiante, estudiante); // va a la funcion mostrarResultados y almacena resultado en success
        
        estudianteAp(definitivaEstudiante, estudiante, success); // va hacia la funcion estudianteAp o estudiante aprovado

        return success; // retorna success o estado aprobado o no
    }
    
    static int mostrarResultados(float definitivaEstudiante[], int estudiante[]) { // Funcion par amostrar resultados de notas y aprobados con reprobados
    ...
    static void mostrarSuccessNoSuccess(int pass, int cantidadEstudiantes) { // funcion para mostrar cantidad de aprobados y reprobados
        System.out.println("\nCantidad Estudianes Aprobados: " + pass);
        System.out.println("Cantidad Estudianes Reprobados: " + (cantidadEstudiantes - pass));
    }

    static void estudianteAp(float definitivaEstudiante[], int estudiante[], int success) { // Funcion para mostrar estudiantes Aprobados
